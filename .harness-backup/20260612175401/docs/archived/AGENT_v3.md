...
## 10. First-Principles Product Interpretation
...
## 19. Vietnamese-Specific Requirements
...
## 28. Questions to Ask Organizers During Training
...
## 29. In-Competition Process

This section describes how the team and system should operate during the actual competition. It is separate from system architecture.

The goal is not only to have good models. The goal is to repeatedly convert judge queries into correct submissions under time pressure.

### 29.1 Before the Competition Session Starts

Checklist:

1. Start all services with a one-command script.
2. Verify visual index is loaded.
3. Verify text/OCR/ASR indexes are loaded.
4. Verify UI can search and preview video.
5. Verify submission/export helper works.
6. Verify official dataset paths and IDs are correct.
7. Verify the scoring/submission interface if available.
8. Open monitoring dashboard/logs.
9. Confirm network status if internet/cloud/API is allowed.
10. Prepare fallback mode if external services fail.
11. Assign human roles.
12. Run 3–5 warm-up queries.

Required readiness signal:

```text
A query can be entered, candidates appear, video preview works, and a result can be copied/submitted.
```

### 29.2 Recommended Human Roles During Live Traditional Mode

For a 5-person team:

| Role | Responsibility |
|---|---|
| Main Operator | Controls UI, opens candidates, submits final answer |
| Query Analyst | Parses the judge query, identifies task type and key constraints |
| Evidence Checker | Verifies candidates using visual/OCR/ASR/caption evidence |
| System Monitor | Watches logs, latency, crashes, service health |
| Backup Operator / Recorder | Tracks query history, failed attempts, and fallback candidates |

For a smaller team:

- one main operator
- one query analyst/evidence checker
- one system/backend monitor if possible

### 29.3 Query Intake Process

When a query is given:

1. Capture the exact query.
2. Assign one of the 4 task types:
   - TKIS
   - VKIS
   - Q&A
   - TRAKE
...
8. Submit only when confidence is high enough, or refine.

### 29.4 TKIS Live Process

For Textual Known-Item Search:

1. Query analyst extracts key visual/event constraints.
2. Operator runs hybrid search.
3. System retrieves visual/caption/object candidates.
4. Evidence checker verifies top candidates.
5. Operator uses timeline expansion around promising candidates.
6. Submit video/timestamp/frame when visual evidence matches.
...
- If top results are visually wrong, try alternate synonyms.
- If query includes text on screen, use OCR.
- If query includes speech/topic, use ASR.
- If event is nearby but not exact, expand ±10–30 seconds.

### 29.5 VKIS Live Process

For Visual Known-Item Search:

1. Determine whether direct visual input is allowed.
2. If allowed, use image/video embedding search.
3. If not allowed, operator/query analyst manually describes:
   - objects
   - people
   - actions
   - colors
   ...
   - visible text
   - layout
4. Run visual or hybrid search.
5. Use similar-frame search from candidates.
6. Submit matching video/timestamp/frame.

Default strategy:

```text
visual clue
→ visual embedding search if allowed
→ near-duplicate / similar-frame retrieval
→ candidate preview
→ timestamp localization
→ submit
```
...
- Convert visual clue into text constraints.
- Search OCR if visible text appears.
- Search objects/colors/actions.
- Use nearby frames to find exact match.

### 29.6 Q&A Live Process

For Q&A:

1. Identify what the question is asking.
2. Determine whether answer likely comes from:
   - speech/ASR
   - OCR
   - visual evidence
   - caption/metadata
   - combination
3. Retrieve evidence first.
4. Generate or select answer only after evidence is found.
5. Attach or remember supporting video/timestamp if required.
6. Submit answer text and evidence fields if official format requires them.
...
→ aggregate ASR/OCR/visual/caption
→ answer synthesis
...
- If ASR is noisy, search semantic transcript and nearby segments.
- If answer depends on slide/screen, search OCR.
- If answer depends on scene, use visual/caption retrieval.
- Do not answer from LLM prior knowledge; answer must be grounded in dataset evidence.

### 29.7 TRAKE Live Process

For TRAKE / temporal reasoning:

1. Identify anchor event.
2. Retrieve anchor event candidate(s).
3. Explore neighboring timeline:
   - before
   - after
   - same shot
   - adjacent shots
4. Identify target event or answer.
5. Verify temporal relation.
6. Submit target timestamp/segment/answer depending on required format.
...
- If anchor is ambiguous, keep multiple anchor candidates.
- Use ASR/OCR/caption to distinguish similar scenes.
- Use timeline view instead of isolated frame view.

### 29.8 Automatic Mode Process

For automatic assistant-vs-assistant mode:

1. Receive query payload.
2. Classify task type.
3. Build structured query plan.
4. Run retrieval tools in parallel.
5. Fuse and rerank.
6. Verify top candidates.
7. Estimate confidence.
8. Return answer within timeout.
9. Log full trace.

Automatic mode must avoid unlimited reasoning loops.
...
- return best candidate with confidence
- include alternatives internally
- do not hallucinate missing evidence
- follow official rules on whether low-confidence answers should be submitted

### 29.9 Submit / Refine / Fallback Decision Policy

Submit when:

- top candidate strongly matches required evidence
- multiple modalities agree
- timestamp is localized
- operator and evidence checker agree
- wrong submissions are not worth risking, if penalty exists

Refine when:

- correct item may be in top-K but not clear
- query has missing constraints
- top results are semantically close but visually wrong
- ASR/OCR evidence conflicts with visual evidence

Fallback when:

- main search service fails
- external model/API fails
- index latency is too high
- UI crashes
- auto-agent exceeds timeout
...
- visual-only search
- local FAISS only
- precomputed captions only
- manual browsing by video/time if necessary
- previous stable build

### 29.10 After Each Query
...
- confidence
- tools used
- whether answer was correct if known
- failure reason if failed
- improvement idea

This creates data for post-session debugging.

### 29.11 Between Competition Sessions
...
- rebuild indexes only if necessary
- re-run validation
- preserve previous stable version

Do not:

- make major untested changes
- change data schema
- delete old indexes
- rely on a newly added model without fallback
- optimize something that did not affect actual failures

### 29.12 In-Competition Priority Order

During competition, prioritize:

1. Correctness
2. Submission validity
3. Speed
4. Evidence confidence
5. System stability
6. Operator clarity

Do not optimize for elegance during the competition.
...
## 33. Engineering Rules for Codex Agents
...
## 34. Minimum Viable System Definition
...
## 35. Current Strategic Conclusion
...
## 9. Official Task Types and Output Expectations

The competition should be modeled around **4 primary task/query types**:

1. **TKIS — Textual Known-Item Search**
2. **VKIS — Visual Known-Item Search**
3. **Q&A — Question Answering over multimedia**
4. **TRAKE — Temporal / reasoning-oriented task**

Important distinction:

- TKIS, VKIS, Q&A, and TRAKE are the main task/query types.
- OCR search, ASR search, caption search, object search, hybrid retrieval, ambiguity handling, and reranking are **internal capabilities**, not separate official query types.
- Traditional mode and automatic mode are **competition modes**, not query types.

### 9.1 Universal Internal Result Object

Even if the official submission format is not yet known, the system should internally represent all answers with a rich structured result object.

The official output adapter can later convert this internal object into the required CSV, JSON, API call, or scoring-server format.
...
```json
{
  ...
  "shot_id": "string_or_null",
  "frame_id": "string_or_null",
  "timestamp_start": 123.45,
  "timestamp_end": 130.20,
  "timestamp": 123.45,
  "answer_text": "string_or_null",
  "confidence": 0.0,
  "modalities_used": ["visual", "caption", "ocr", "asr", "metadata"],
  "evidence": {
    "caption": "string_or_null",
    "ocr_text": "string_or_null",
    "asr_text": "string_or_null",
    "objects": ["string"],
    "visual_notes": "string_or_null",
    "temporal_notes": "string_or_null"
  },
  "scores": {
    "visual": 0.0,
    "caption": 0.0,
...
### 9.2 TKIS — Textual Known-Item Search

Input:

- A natural-language text description of a target moment, event, object, scene, or situation.

Expected task:

- Find the exact or best-matching item in the multimedia database.

Primary output expectation:

- `video_id`
- `timestamp` or `timestamp_start/timestamp_end`
- possibly `frame_id` or `segment_id`, depending on official rules
...
```json
{
  ...
  "confidence": 0.86,
  "evidence": {
    "caption": "A man in a red shirt stands next to a bus.",
    "objects": ["person", "bus"],
    "visual_notes": "red shirt, bus nearby"
  }
}
```
...
- OCR if visible text is part of the clue
- ASR if spoken content is part of the clue
- query expansion
- fusion and reranking
- temporal localization

### 9.3 VKIS — Visual Known-Item Search

Input:

- A visual clue, such as an image, frame, or short video snippet.
- If direct visual input is restricted, the human operator may need to convert the visual clue into textual constraints.

Expected task:

- Find the matching or near-matching video/frame/segment in the database.
...
```json
{
  ...
  "confidence": 0.91,
  "evidence": {
    "visual_match_score": 0.94,
    "caption": "A visually similar scene with a woman standing near a white car."
  }
}
```
...
TBD from official rules:

- whether teams can directly input the shown image/video into the system
- whether teams are forbidden from capturing the visual clue
- whether the operator must manually describe the visual clue

### 9.4 Q&A — Question Answering Over Multimedia

Input:

- A natural-language question whose answer must be derived from multimedia evidence.

Expected task:

- Retrieve relevant evidence and produce an answer grounded in video/image/audio/text/OCR/transcript/metadata.

Primary output expectation:

- `answer_text`
- likely supporting `video_id`
- likely timestamp or evidence reference if required
...
```json
{
  ...
  "evidence": {
    ...
    "caption": "A speaker is presenting on stage.",
...
- retrieval over visual/caption/OCR/ASR/metadata
- evidence aggregation
- answer synthesis
- confidence scoring
- evidence verification
...
- whether answer text alone is accepted
- whether evidence must be submitted
- whether exact wording matters
- whether answer is multiple-choice, free-text, or retrieval-based

### 9.5 TRAKE — Temporal / Reasoning Task

Input:

- A query involving event sequence, before/after relationships, transitions, temporal context, or reasoning across multiple segments.

Expected task:

- Identify a target event/segment or answer a question based on temporal and multimodal evidence.

Primary output expectation:

- `video_id`
- timestamp or segment range
- possibly `answer_text` depending on the exact TRAKE format
...
```json
{
  ...
  "evidence": {
    ...
    "caption": "A person is standing at a reception desk.",
    "temporal_notes": "Target event occurs immediately after the entrance event."
  }
}
```
...
- timeline navigation
- temporal expansion
- adjacent shot search
- event sequence modeling
- multimodal evidence aggregation
- before/after reasoning
- visual + ASR + OCR timeline alignment

TBD from official rules:

- official definition of TRAKE in AI Challenge 2026
- whether output is a timestamp, segment, textual answer, or combination
- whether multi-step reasoning evidence is required

### 9.6 Supporting Retrieval Signals and Capabilities

These are **not official query types**. They are internal capabilities used to solve TKIS, VKIS, Q&A, and TRAKE.

| Capability | Used For |
|---|---|
| OCR search | visible text, signs, slides, banners |
| ASR/transcript search | spoken content, audio topics, interviews |
| Caption search | semantic visual description |
| Object/concept search | explicit objects, people, vehicles, places |
| Hybrid multimodal retrieval | combining visual/audio/text/OCR evidence |
| Query expansion | Vietnamese-English synonyms and related terms |
| Ambiguity handling | vague natural-language clues |
| Reranking | improving top-K candidate ordering |
| Evidence verification | reducing false positives |
| Confidence scoring | deciding submit/refine/fallback behavior |

### 9.7 Competition Modes Are Not Query Types

The two competition modes are:

1. **Traditional human-in-the-loop mode**
2. **Automatic assistant-vs-assistant mode**

Both modes may contain TKIS, VKIS, Q&A, and TRAKE tasks.

The same retrieval core should serve both modes.
