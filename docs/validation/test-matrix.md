...
| SYS1-003 | Notebook 01 fixed production pipeline produces TransNet shots, search-band keyframes, faster-whisper ASR, Gemini bilingual captions/scene boundaries/summaries, strict packages, and resumable sync | yes | partial | no | no | partial | The 247-test local suite covers config hashes, batch-scoped Phase00 restore/corruption recovery, grouped checkpoint promotion/invalidation, shot partitions, bounded-RAM keyframe selection, ASR/OOM policy, Gemini retry/schema cache, scene voting, package validation, and QA sampling. Real-provider/Colab execution remains pending and TransNet artifact provisioning is a preflight blocker. |
