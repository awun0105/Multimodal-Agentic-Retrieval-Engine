...
| Area | State | Evidence |
| --- | --- | --- |
| Canonical per-video metadata in Notebook 00B/00C | accepted target; not implemented | ADR 0016 requires one normalized JSON per video, preserved organizer source, `ffprobe` facts, provenance propagation, and validation. Current package still emits minimal placeholders and keeps probe facts only in inventory. |
