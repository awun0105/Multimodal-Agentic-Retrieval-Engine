...
| Shot captioning | `shot_captions` rows | Phase01 keeps one canonical row per shot. Static/simple shots use the representative image; eligible changing shots use one bounded deterministic storyboard from canonical keyframes. Shared Qwen/Vintern plain-text field requests are cached, retried, resumable, versioned, and backed by exact source-frame provenance; persistent production failure fails the video. |
